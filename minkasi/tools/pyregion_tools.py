...
def region_binner(shapeList, hdu, plot=False, return_rs=True):
    ...
    if len(shapeList) > 1:
        ...
        return _region_binner_explicit_regions(
            shapeList, hdu, plot=plot, return_rs=return_rs
        )
    ...
    if return_rs:
        ...
        for i in range(len(rs)):
            if shapeList[i].name == "epanda":
                ...
                rs[i] = (
                    np.mean([shapeList[i].coord_list[5], shapeList[i].coord_list[7]])
                    * cdelt
                    * 3600
                )
                ...
                print(
                    "Error: region type {} not supported for automatic radii generation".format(
                        shapeList[i].name
                    )
                )
                return means, var
