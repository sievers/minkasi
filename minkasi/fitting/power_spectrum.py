...
def fit_linear_ps_uncorr(dat, vecs, tol=1e-3, guess=None, max_iter=15):
    if guess is None:
        ...
        guess = (
            0.5 * guess
        )  # scale down since we're less likely to run into convergence issues if we start low
        # print guess
    fitp = guess.copy()
    converged = False
    npp = len(fitp)
    ...
    while converged == False:
        ...
        for i in range(npp):
            ...
            for j in range(i, npp):
                ...
                curve[i, j] = 0.5 * np.sum(
                    Cinv * Cinv * vecs[i, :] * vecs[j, :]
                ) - np.sum(
                    datsqr * Cinv * Cinv * Cinv * vecs[i, :] * vecs[j, :]
                )  # exact
                curve[j, i] = curve[i, j]
        ...
        if np.max(np.abs(frac_shift)) < tol:
            print(
                "successful convergence after ",
                iter,
                " iterations with error estimate ",
                np.max(np.abs(frac_shift)),
            )
            converged = True
            print(C[0], C[-1])
        if iter == max_iter:
            print(
                "not converging after ",
                iter,
                " iterations in fit_linear_ps_uncorr with current convergence parameter ",
                np.max(np.abs(frac_shift)),
            )
            converged = True
...
def get_curve_deriv_powspec(fitp, nu_scale, lognu, datsqr, vecs):
    ...
    for i in range(np):
        for j in range(i, np):
            curve[i, j] = 0.5 * np.sum(Cinv * Cinv * vecs[i, :] * vecs[j, :]) - np.sum(
                datsqr * Cinv * Cinv * Cinv * vecs[i, :] * vecs[j, :]
            )
            curve[j, i] = curve[i, j]
...
def fit_ts_ps(
    ...
    for iter in range(50):
        ...
        frac = dp / errs
        new_like, new_grad, new_curve, C = get_curve_deriv_powspec(
            trial_fitp, nu_scale, lognu, datsqr, vecs
        )
        ...
        if False:
            ...
            for i in range(np):
                ...
                for j in range(i, np):
                    curve[i, j] = 0.5 * np.sum(
                        Cinv * Cinv * vecs[i, :] * vecs[j, :]
                    ) - np.sum(datsqr * Cinv * Cinv * Cinv * vecs[i, :] * vecs[j, :])
                    curve[j, i] = curve[i, j]
            grad = grad_chi + grad_tr
            curve_inv = np.linalg.inv(curve)
            errs = np.diag(curve_inv)
