...
class WavSkyMap(SkyMap):
    ...
    def get_response_matrix(self, todvec):
        ...
        for nfilt, filt in enumerate(self.needlet.filters):
            ...
            to_ret_cur = np.zeros((nxs_red * nys_red, self.nx * self.ny))
            ...
            if have_mpi:
                if myrank == 0:
                    ...
                    for nx in range(myrank - 1, nxs_red, nproc - 1):
                        for ny in range(nys_red):
                            idx = nys_red * nx + ny
                            unit_impulse = np.zeros((self.nx, self.ny))
                            unit_impulse[nx_space[nx], ny_space[ny]] = 1 #Make a map in wavelet space that is one at only one place
                            unit_impulse = unit_impulse[None,] #Dummy axis

                            m_wav2map = wav2map_real(unit_impulse, filt) #What is the mapspace response of that wavelet
                            mapset.maps[0].map[:] = m_wav2map

                            map_acum = todvec.dot(mapset) #This gets the noise response of the tods to 
                                                                #the realspace map generated by the unit impulse
                                                                #wavelet
                            temp = map2wav_real(map_accum.map, filter_bank) #ideally this would be course. Send noise
                                                                            #response back to wave space
                            comm.send(temp, dest=0, tag=idx)
                    comm.send("Done", dest=0, tag=0)
                ...
                for nx in range(nxs_red):
                    for ny in range(nys_red):
                        ...
                        m_wav2map = wav2map_real(unit_impulse, filt) #What is the mapspace response of that wavelet
                        real_map = self.real_map.copy()
                        real_map.map[:] = m_wav2map

                        map_acum = todvec.dot(real_map) #This gets the noise response of the tods to
                                                            #the realspace map generated by the unit impulse
                                                            #wavelet
                        temp = map2wav_real(map_accum.map, filter_bank)
                        to_ret_cur[idx] = temp
        return to_ret
