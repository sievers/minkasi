...
class WavSkyMap(SkyMap):
    ...
    def get_response_matrix(self, todvec):
        ...
        for nfilt, filt in enumerate(self.needlet.filters):
            ...
            to_ret_cur = np.empty((nxs_red * nys_red, self.needlet.nfilt, self.nx , self.ny))
             
           
            real_map = self.real_map.copy()
            mapset = Mapset()
            mapset.add_map(real_map)
            for nx in range(nxs_red): #TODO: make numba prange
                for ny in range(nys_red):
                    idx = nys_red * nx + ny
                    toc = time.time()
                    ...
                    map_accum = todvec.dot(mapset) #This gets the noise response of the tods to
                                                        #the realspace map generated by the unit impulse
                                                        #wavelet
                    temp = map2wav_real(map_accum.maps[0].map, self.needlet.filters) #TODO: Buffer
                    
                    to_ret_cur[idx] = temp[0,...]
                    tic = time.time()
