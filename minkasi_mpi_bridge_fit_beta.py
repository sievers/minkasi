#this is a (very crude!) sample script to demonstrate multi-component model fitting
...
if os.getenv('HOME').find('Users')>0:
    print('working on laptop')
    dir='../data/bridge/'
    act_dir='/Users/sievers/actpol/pixel_noise/'
    act_prof=np.loadtxt('beams/s17_pa6_f090_nohwp_night_beam_profile_jitter.txt')
else:
    print('working on niagara')
    dir='../data/bridge/TS_EaCMS0f0_51_21_Jan_2020/'
    act_dir='./act_priors/'
    act_prof=np.loadtxt('/project/r/rbond/sigurdkn/actpol/map_coadd/20200228/inputs/beams/s17_pa5_f090_nohwp_night_beam_profile_jitter.txt')
...
pars_401=np.asarray([7.80824801e-01,  2.36971606e-01,  5.31403879e-04,  3.60679159e-01,-4.60194148e+03])
...
pars_fit,chisq,curve=minkasi.fit_timestreams_with_derivs_manyfun(funcs,pars,npar_func,todvec,maxiter=5)
if minkasi.myrank==0:
    np.save('bridge_pars.npy',pars_fit)
    np.save('bridge_curve.npy',curve)
...
model.clear()
for tod in todvec.tods:
    pred=minkasi.timestreams_from_isobeta_c(pars,tod)
    model.tod2map(tod,pred)
model.mpi_reduce()


mask=hits.map>0
model.map[mask]=model.map[mask]/hits.map[mask]
if minkasi.myrank==0:
    model.write('bridge_model.fits')
assert(1==0)
